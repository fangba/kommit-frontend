...
- Les **livrables des subagents lancés**, sur le disque :
  - `unit-test-writer` → `src/logic/**/*.test.ts` (Vitest, rouges) ;
  - `smoke-test-writer` → `testing/smokes-<TICKET_ID>.md` (scénarios navigateur manuels).
...
- **En parallèle, dans un seul message.** Leurs périmètres d'écriture sont disjoints (🟢 + part unitaire des 🟠 pour `unit-test-writer` ; 🔴 + complément des 🟠 pour `smoke-test-writer`), donc c'est gratuit.
