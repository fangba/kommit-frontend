...
- Le **ticket Notion associé**, déduit du nom de branche (`FE\d+`), à passer en `TESTING`.
...
- **Propriété ID ticket** : `Ticket ID` (texte, ex. `FE01`)
- **Propriété statut** : `Agile Statut` → option cible `TESTING`

## ⛔️ Garde-fou merge (NON négociable)

La rule Git (globale et du projet) interdit tout merge sans accord explicite **pour cette PR précise**.
...
2. **Passer le ticket en `TESTING`.**
   - Identifier le ticket depuis le nom de branche : extraire le token `FE\d+` (ex. `FE01-authentification` → `FE01`), chercher dans le Backlog Général le ticket dont `Ticket ID` (texte) vaut ce token exact. Charger l'outil Notion via ToolSearch si besoin (`notion-update-page`).
...
```
✅ PR #42 mergée (branche supprimée) → ticket FE01 en TESTING
```
